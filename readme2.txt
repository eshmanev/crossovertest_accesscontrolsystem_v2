...
1.4. Copy and replace .erlang.cookie file from Artifacts folder to %WinDir%\ServiceProfiles\NetworkService and %WinDir%\.
1.5. Open the following ports:
...
1.6. Allow connection for two binaries %Program Files%\erl7.1\bin\erl.exe and %Program Files%\erl7.1\erts-7.1\bin\erl.exe
...
The following steps should be executed on any node in your cluster
1.7. Run RabbitMQ Command prompt, join the cluster and set mirroring policy
...
1.8. Run RabbitMQ Command prompt and enable RabbitMQ management tool
